...
private:
	...
	void encodeTable(lua_State* L, Writer* writer, int idx, int depth)
	{
		...
		lua_pushvalue(L, idx); // [table]
		values::json_table_type table_type = values::getJsonType(L, -1);
		if (table_type == values::json_table_type::JSON_TABLE_TYPE_ARRAY)
		{
		...
		if (!sort_keys)
		{
			if (table_type == values::json_table_type::JSON_TABLE_TYPE_OBJECT_NUM_KEY)
			{
				encodeObjectNumKey(L, writer, depth);
			}
			else
			{
	...
	template<typename Writer>
	void encodeObjectNumKey(lua_State* L, Writer* writer, int depth)
	{
		writer->StartObject();
		writer->Key("__numkey", 8);
		writer->Int(1);
		...
		while (lua_next(L, -2))
		{
			...
			else if (type == LUA_TNUMBER)
			{
				...
				{
        			const char* end = internal::i64toa(integer, key);
					len = end - key;
				}
				else
				{
					double number = lua_tonumber(L, -2);
					if (!internal::Double(number).IsNanOrInf()) {
						char* end = internal::dtoa(number, key);
						len = end - key;
					} else {
						luaL_error(L, "invalid number key(nan or inf)");
					}
				}
				writer->Key(key, static_cast<SizeType>(len));
				encodeValue(L, writer, -1, depth);
...
#if defined(_DEBUG)
#define OPEN_API_NAME luaopen_rapidjson
#else						
#define OPEN_API_NAME luaopen_rapidjson
#endif	
...
LUALIB_API int OPEN_API_NAME(lua_State* L)
{
	...
	createSharedMeta(L, "json.object_numkey", "object_numkey");
